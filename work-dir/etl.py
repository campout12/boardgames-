...
from pyspark.sql.functions import row_number, lit, when, col, split , explode, trim
...
bgg_df = spark.read.csv("/opt/spark/files/in/bgg_ranks.csv", header=True, inferSchema=True)
mmbg_df = spark.read.csv("/opt/spark/files/in/melissa-monfared-board-games.csv", header=True, inferSchema=True, encoding="cp1252") # change encoding if needed
...
#data cleaning
full_df = full_df.dropDuplicates().dropna(subset=["game_name", "min_players", "max_players", "avg_rating"])
...
#DOMAIN TABLE
domain_df.write.format("jdbc").options(**jdbc_options,dbtable="domain").model("append").save()

#MECHANICS TABLE
mechanics_df.write.format("jdbc").options(**jdbc_options,dbtable="mechanics").model("append").save()

#BOARD GAME DOMAINS BRIDGE TABLE
board_game_domains_df.write.format("jdbc").options(**jdbc_options,dbtable="board_game_domains").model("append").save()

#BOARD GAME MECHANICS BRIDGE TABLE
board_game_mechanics_df.write.format("jdbc").options(**jdbc_options,dbtable="board_game_mechanics").model("append").save()
